...
for reg in regions:
    ...
    aff_exp = aff_exp.sort_values(['nhh'], ascending=False)
    
    n_aff_hh = aff_exp['nhh'].sum()
    print(n_aff_hh)
    ...
    for centr in aff_ids:
        ...
        n_hhs_cntr = aff_exp.loc[aff_exp['centroid_id']==centr,'nhh'].sum()
        c_hh = 0
        hh_pack_size = 1
        while c_hh < n_hhs_cntr:
            if n_hhs_cntr-c_hh <hh_pack_size:
                hh_pack_size = n_hhs_cntr-c_hh
            ...
            aff_hh_data.iloc[c_aff_hhs,:].loc['weight'] =  hh_pack_size
            ...
            reg_data.loc[:,'weight'].iloc[index_counter] -= hh_pack_size
    ...
    reg_data = reg_data.reset_index().drop(columns=['index'])
    reg_data.to_csv('/home/insauer/projects/WB_model/hhwb/data/surveys_prepared/PHL/region_hh_30as_{}_pack.csv'.format(str(reg)))
